...
public:
	...
	CEdge* m_pDijkstraPrevious = NULL;
	...
	// Atributs per Prim
	bool m_Pertany = false;
	// Atributs per Backtracking
	bool m_PerVisitar = false;
	list<int> m_Trams;
...
public:
	...
	bool m_Processed;
	// Backtracking basado en aristas
	bool m_Used = false; // Ya se ha usado la arista en el camino
public:
...
class CTrack {
public:
	// Elements for Branch and Bound added by me
	int lastIndex = 0;
	vector<int> indexes;
	double length;
	...
	CTrack(const CTrack& t) : m_pGraph(t.m_pGraph), m_Edges(t.m_Edges), lastIndex(t.lastIndex), indexes(t.indexes), length(t.length) {} // Last 3 added by me
	CTrack operator=(const CTrack& t) {
		m_pGraph = t.m_pGraph;
		m_Edges = t.m_Edges;
		lastIndex = t.lastIndex; // Added by me
		indexes = t.indexes; // Added by me
		length = t.length; // Added by me
		return *this;
	}
